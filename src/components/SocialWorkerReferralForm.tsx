// src/components/SocialWorkerReferralIntakeForm.tsx
import React, { useMemo, useState } from "react";

type Timing = "ASAP" | "Within 2 weeks" | "2–6 weeks" | "1–3 months" | "Unsure";
type CurrentLocation = "Home" | "Hospital" | "Other";

type PlacementNeed = "Respite" | "Respite with a view to permanent" | "Permanent care" | "";
...
// referrer role
type ReferrerRole = "Social worker" | "Discharge planner" | "Case manager" | "Other" | "";
...
type FormData = {
  // Referrer (social worker / hospital team)
  referrerName: string;
...
// Your Web3Forms key (same as FollowUpForm)
...
export default function SocialWorkerReferralIntakeForm() {
  ...
  const canSubmit = useMemo(() => {
    // Referrer required
    if (!data.referrerName.trim()) return false;
    if (!data.referrerRole) return false;
    if (!data.organisation.trim()) return false;
    if (!data.referrerPhone.trim()) return false;
    if (!data.referrerEmail.trim()) return false;

    // Intake required (same spirit as FollowUpForm)
  ...
  const buildSummary = () => {
    ...
    lines.push(`ROLE: ${data.referrerRole}`);
    lines.push(`ORGANISATION: ${data.organisation}`);
    ...
    if (data.ownHome) lines.push(`DO THEY OWN A HOME?: ${data.ownHome}`);
    if (data.ownHome === "Yes") {
    ...
    if (data.govtHelpAccommodation)
      lines.push(`GOV HELP FOR ACCOMMODATION COSTS: ${data.govtHelpAccommodation}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        subject: `NHNM – Social Worker Referral – ${data.preferredSuburbArea || "Area"} – ${
          data.timing || "Timing"
        }`,
        from_name: "NursingHomesNearMe",
        // helpful: reply to referrer
      ...
      if (json.success) {
        setStatus({ type: "success", message: "Submitted. Thank you — we’ll follow up shortly." });
        ...
        setData((prev) => ({
          ...
          organisation: "",
          referrerPhone: "",
          // keep email if you want, but resetting is cleaner:
          referrerEmail: "",
  ...
  // consistent form control sizing (same as FollowUpForm)
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="mx-auto w-full max-w-3xl px-4 py-10">
        ...
        <h1 className="text-2xl font-semibold text-slate-900">Social Worker Referral</h1>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          ...
          <section className="rounded-xl border border-slate-200 p-5">
            <h2 className="text-lg font-semibold text-slate-900">Referrer details</h2>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setField("referrerRole", e.target.value as FormData["referrerRole"])}
                >
              ...
              <div className="md:col-span-2">
                <label className="text-sm font-medium text-slate-700">Organisation / Hospital *</label>
                <input
                  className={inputClass}
                  value={data.organisation}
                  onChange={(e) => setField("organisation", e.target.value)}
                  placeholder="e.g. GCUH, Robina Hospital, Community Health"
                />
              </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setField(
                      "relationshipToPatient",
                      e.target.value as FormData["relationshipToPatient"]
                    )
                  }
                >
                  <option value="">Select…</option>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setField("maritalStatus", e.target.value as FormData["maritalStatus"])
                  }
                >
                  <option value="">Select…</option>
              ...
              <div className="md:col-span-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setField("pensionStatus", e.target.value as FormData["pensionStatus"])
                  }
                >
                  <option value="">Select…</option>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="md:col-span-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setField("livingSituation", e.target.value as FormData["livingSituation"])
                  }
                >
                  <option value="">Select…</option>
              ...
              <div>
                <label className="text-sm font-medium text-slate-700">Do they own their home?</label>
                <select
                  className={selectClass}
                  value={data.ownHome}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setField(
                          "someoneLivingInHome",
                          e.target.value as FormData["someoneLivingInHome"]
                        )
                      }
                    >
                      <option value="">Select…</option>
          ...
          <section className="rounded-xl border border-slate-200 p-5">
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <label className="text-sm font-medium text-slate-700">Have they had an ACAT? *</label>
                <select
                  className={selectClass}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setField(
                      "govtHelpAccommodation",
                      e.target.value as FormData["govtHelpAccommodation"]
                    )
                  }
                >
                  <option value="">Select…</option>
          ...
          <section className="rounded-xl border border-slate-200 p-5">
            ...
            <label className="mt-4 flex items-start gap-3 text-sm text-slate-700">
              ...
              />
              <span>
                I confirm I have consent to share these details with nursing homes to check availability and arrange call-backs. *
              </span>
            </label>
          </section>
